...
const CreatePodcast = () => {
  ...
  const [aiAudioUrl, setAiAudioUrl] = useState<File | null>(null);
  const [aiThumbnailUrl, setAiThumbnailUrl] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    // --- AUDIO FILE ---
    // Always fetch the audio as a blob from aiAudioUrl (if it's a string URL)
    if (typeof aiAudioUrl === 'string') {
      const audioBlob = await fetch(aiAudioUrl).then(r => r.blob());
      formData.append("audioFile", audioBlob, "podcast-audio.wav");
    }

    // --- IMAGE FILE ---
    // Use customImage if present, else fetch blob from aiThumbnailUrl (if it's a string URL)
    if (customImage) {
      formData.append("imageFile", customImage, customImage.name);
    } else if (typeof aiThumbnailUrl === 'string') {
      const imageBlob = await fetch(aiThumbnailUrl).then(r => r.blob());
      formData.append("imageFile", imageBlob, "podcast-thumbnail.png");
    }
    ...
    // When creating the newPodcast object for Zustand, use the string URLs, not File objects
    const newPodcast = {
      title: podcastTitle,
      category,
      description,
      audioUrl: typeof aiAudioUrl === 'string' ? aiAudioUrl : '',
      thumbnailUrl: typeof aiThumbnailUrl === 'string' ? aiThumbnailUrl : '',
    ...
    <div className="flex flex-col min-h-[calc(100vh-64px)] max-h-[calc(100vh-64px)] overflow-y-auto">
      ...
      >
        ...
        <div className="mb-4">
          ...
          {aiAudioUrl && (
            <audio ref={audioRef} controls src={typeof aiAudioUrl === 'string' ? aiAudioUrl : undefined} className="mt-2 w-full" />
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-[#2A2D36] rounded-lg py-8 bg-[#181A20] min-h-[120px]">
            {aiThumbnailUrl && (
              <img src={typeof aiThumbnailUrl === 'string' ? aiThumbnailUrl : undefined} alt="Thumbnail" className="max-h-auto rounded mb-2 w-95" />
            )}
            {!aiThumbnailUrl && (
