...
const CreatePodcast = () => {
  ...
  const { user } = useUser(); // Only use 'user', remove unused 'isLoaded'
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    if (customImage) {
      ...
      imageFile = new File([imageBlob], "podcast-thumbnail.png", { type: imageBlob.type });
    }
    if (!imageFile) {
      toast.error("No image file available for upload");
      return;
    }
    const audioBlob = await fetch(aiAudioUrl).then(r => r.blob());
    const audioFile = new File([audioBlob], "podcast-audio.wav", { type: audioBlob.type });

    // Prepare form data
    const formData = new FormData();
    formData.append("userId", user.id); // Use Clerk user ID
    ...
    try {
      ...
      toast.success("Podcast created and uploaded!");
      // Optionally: addPodcast(res.data.podcast) to Zustand
    } catch (err) {
